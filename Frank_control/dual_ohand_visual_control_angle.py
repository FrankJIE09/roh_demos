...
def map_value(value, in_min, in_max, out_min, out_max):
    ...
    if out_min <= out_max:
        ...
        return max(min(mapped, out_min), out_max)
...
def send_to_ohand_individual(client: OHandModbusClient | None, target_positions: list):
    ...
    try:
        ...
        print(f"Error sending OHand command (Client: {client.client.port if client else 'N/A'}): {e}")
...
def load_calibrated_ranges(filename, defaults):
    ...
    try:
        ...
        for bend_key in ["Index Bend", "Middle Bend", "Ring Bend", "Pinky Bend"]:
            min_val = get_val(bend_key, "min");
            max_val = get_val(bend_key, "max")
            if min_val < max_val: bend_mins.append(min_val); bend_maxs.append(max_val)
        thumb_bend_min = get_val("Thumb Bend", "min");
        thumb_bend_max = get_val("Thumb Bend", "max")
        thumb_rot_min = get_val("Thumb Rotation", "min");
        thumb_rot_max = get_val("Thumb Rotation", "max")
        thumb_rot_min_left = get_val("Thumb Rotation Left", "min");
        thumb_rot_max_left = get_val("Thumb Rotation Left", "max")
        ...
        if bend_straight < bend_bent:
            ...
            print(f"  Warning: Loaded combined Bend range invalid. Using defaults.")
        ...
        if thumb_bend_min < thumb_bend_max:
            ...
            print(f"  Warning: Loaded Thumb Bend range invalid. Using defaults.")
        ...
        if thumb_rot_min < thumb_rot_max:
            ...
            print(f"  Warning: Loaded Thumb Rot range invalid. Using defaults.")

        if thumb_rot_min_left < thumb_rot_max_left:
            final_ranges["THUMB_ROT_ANGLE_MIN_Left"] = thumb_rot_min_left
            final_ranges["THUMB_ROT_ANGLE_MAX_Left"] = thumb_rot_max_left
            print(f"  Loaded Thumb Rot range: {thumb_rot_min:.2f} - {thumb_rot_max:.2f}")
        else:
            print(f"  Warning: Loaded Thumb Rot range invalid. Using defaults.")
        return final_ranges
        ...
        print(f"Calibration file '{filename}' not found. Using default angle ranges."); return defaults
    except (json.JSONDecodeError, TypeError, KeyError) as e:
        print(f"Error parsing calibration file {filename}: {e}. Using default angle ranges."); return defaults
    except Exception as e:
        print(f"Unknown error loading JSON: {e}. Using default angle ranges."); return defaults
...
def main():
    ...
    THUMB_ROT_ANGLE_MIN_Left = active_angle_ranges["THUMB_ROT_ANGLE_MIN_Left"]
    THUMB_ROT_ANGLE_MAX_Left = active_angle_ranges["THUMB_ROT_ANGLE_MAX_Left"]
    ...
    # --- Initialize Devices ---
    if ohand_left_client is None and ohand_right_client is None: print(
        "Error: Both OHand clients failed to initialize. Exiting."); return
    orbbec_camera = None
    ...
    try:
        ...
        if ohand_left_client:
            ...
            if ohand_left_client.connect():
                ohand_left_connected = True; print("Left OHand connected.")
            else:
                print(f"Error: Failed to connect Left OHand ({OHAND_LEFT_PORT}).")
        if ohand_right_client:
            print(f"Connecting Right OHand ({OHAND_RIGHT_PORT})...")
            if ohand_right_client.connect():
                ohand_right_connected = True; print("Right OHand connected.")
            else:
                print(f"Error: Failed to connect Right OHand ({OHAND_RIGHT_PORT}).")
        if not (ohand_left_connected or ohand_right_connected): print(
            "Warning: No OHand grippers connected. Visual tracking only.")
        ...
        while True:
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                    ...
                    label_pos = (int(hand_landmarks.landmark[0].x * image_width) - 10,
                                 int(hand_landmarks.landmark[0].y * image_height) - 20)
                    cv2.putText(color_image, f"{display_label} ({score:.2f})", label_pos, FONT_FACE, FONT_SCALE,
                                display_color, FONT_THICKNESS)
                    ...
                    try:
                        ...
                        bend_metrics = {}
                        bend_metrics["Index"] = (180.0 - calculate_angle_3d(
                            ...
                            landmarks[mp.solutions.hands.HandLandmark.INDEX_FINGER_DIP])) + \
                                                (180.0 - calculate_angle_3d(
                                                    landmarks[mp.solutions.hands.HandLandmark.INDEX_FINGER_PIP],
                                                    landmarks[mp.solutions.hands.HandLandmark.INDEX_FINGER_DIP],
                                                    landmarks[mp.solutions.hands.HandLandmark.INDEX_FINGER_TIP]))
                        bend_metrics["Middle"] = (180.0 - calculate_angle_3d(
                        ...
                        bend_metrics["Ring"] = (180.0 - calculate_angle_3d(
                        ...
                        bend_metrics["Pinky"] = (180.0 - calculate_angle_3d(
                        ...
                        bend_metrics["Thumb"] = (180.0 - calculate_angle_3d(
                            landmarks[mp.solutions.hands.HandLandmark.THUMB_CMC],
                        ...
                        thumb_rot_angle = calculate_thumb_rotation_xy_angle(
                            landmarks[mp.solutions.hands.HandLandmark.WRIST],
                            landmarks[mp.solutions.hands.HandLandmark.THUMB_MCP],
                            landmarks[mp.solutions.hands.HandLandmark.INDEX_FINGER_MCP])

                        print(thumb_rot_angle)
                        # Map angles to OHand positions using loaded/default ranges
                        target_hand_raw_positions[FINGER_INDEX] = map_value(bend_metrics["Index"], BEND_ANGLE_STRAIGHT,
                                                                            BEND_ANGLE_BENT, OHAND_POS_OPEN,
                                                                            OHAND_POS_CLOSED)
                        target_hand_raw_positions[FINGER_MIDDLE] = map_value(bend_metrics["Middle"],
                                                                             BEND_ANGLE_STRAIGHT, BEND_ANGLE_BENT,
                                                                             OHAND_POS_OPEN, OHAND_POS_CLOSED)
                        target_hand_raw_positions[FINGER_RING] = map_value(bend_metrics["Ring"], BEND_ANGLE_STRAIGHT,
                                                                           BEND_ANGLE_BENT, OHAND_POS_OPEN,
                                                                           OHAND_POS_CLOSED)
                        target_hand_raw_positions[FINGER_PINKY] = map_value(bend_metrics["Pinky"], BEND_ANGLE_STRAIGHT,
                                                                            BEND_ANGLE_BENT, OHAND_POS_OPEN,
                                                                            OHAND_POS_CLOSED)
                        target_hand_raw_positions[FINGER_THUMB_BEND] = map_value(bend_metrics["Thumb"],
                                                                                 THUMB_BEND_ANGLE_STRAIGHT,
                                                                                 THUMB_BEND_ANGLE_BENT, OHAND_POS_OPEN,
                                                                                 OHAND_POS_CLOSED)
                        if handedness == 'Left':
                            target_hand_raw_positions[FINGER_THUMB_ROT] = map_value(thumb_rot_angle,
                                                                                    THUMB_ROT_ANGLE_MIN,
                                                                                    THUMB_ROT_ANGLE_MAX,
                                                                                    OHAND_THUMB_ROT_MIN,
                                                                                    OHAND_THUMB_ROT_MAX)
                            ...
                            target_hand_raw_positions[FINGER_THUMB_ROT] = map_value(thumb_rot_angle,
                                                                                    THUMB_ROT_ANGLE_MIN_Left,
                                                                                    THUMB_ROT_ANGLE_MAX_Left,

                                                                                    OHAND_THUMB_ROT_MAX,OHAND_THUMB_ROT_MIN)

                    except Exception as calc_e:
                        print(f"Error calculating/mapping angles: {calc_e}")

                    # Update raw targets based on handedness (swapped logic)
                    if handedness == 'Left':
                        raw_target_positions_right = target_hand_raw_positions[:]; detected_right_this_frame = True
                    elif handedness == 'Right':
                        raw_target_positions_left = target_hand_raw_positions[:]; detected_left_this_frame = True

            # 3a. Apply EMA Smoothing
            for i in range(6):
                if detected_left_this_frame: smoothed_positions_left[i] = SMOOTHING_FACTOR * raw_target_positions_left[
                    i] + (1 - SMOOTHING_FACTOR) * smoothed_positions_left[i]
                if detected_right_this_frame: smoothed_positions_right[i] = SMOOTHING_FACTOR * \
                                                                            raw_target_positions_right[i] + (
                                                                                        1 - SMOOTHING_FACTOR) * \
                                                                            smoothed_positions_right[i]
            ...
            if detected_left_this_frame:
                for i in range(6):
                    if abs(round(smoothed_positions_left[i]) - round(
                        last_sent_positions_left[i])) > POS_CHANGE_THRESHOLD: left_pos_changed = True; break
            right_pos_changed = False
            if detected_right_this_frame:
                for i in range(6):
                    if abs(round(smoothed_positions_right[i]) - round(
                        last_sent_positions_right[i])) > POS_CHANGE_THRESHOLD: right_pos_changed = True; break
            ...
            if should_send or left_pos_changed or right_pos_changed:
                ...
                if should_send and not send_triggered:
                    if ohand_left_connected and detected_left_this_frame: send_to_ohand_individual(ohand_left_client,
                                                                                                   smoothed_positions_left); last_sent_positions_left = smoothed_positions_left[
                                                                                                                                                        :]
                    if ohand_right_connected and detected_right_this_frame: send_to_ohand_individual(ohand_right_client,
                                                                                                     smoothed_positions_right); last_sent_positions_right = smoothed_positions_right[
                                                                                                                                                            :]
    ...
    # --- Exception Handling & Cleanup ---
        ...
        if orbbec_camera is not None:
            ...
            try:
                ...
                print(f"Error stopping Orbbec camera: {e}")
        if ohand_left_client is not None:
            print("Disconnecting Left OHand...")
            try:
                ohand_left_client.disconnect()
            except Exception as e:
                print(f"Error disconnecting Left OHand: {e}")
        if ohand_right_client is not None:
            print("Disconnecting Right OHand...")
            try:
                ohand_right_client.disconnect()
            except Exception as e:
                print(f"Error disconnecting Right OHand: {e}")
