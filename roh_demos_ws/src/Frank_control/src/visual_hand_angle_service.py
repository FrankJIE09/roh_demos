...
class HandAngleServiceNode(object):
    def __init__(self):
        ...
        # 摄像头初始化
        sn = self.config['ORBBEC_CAMERA_SERIAL_NUMBER']
        if not sn:
            sns = get_serial_numbers()
            sn = sns[0] if sns else ''
        self.camera = OrbbecCamera(sn)
        self.camera.start_stream(depth_stream=True, color_stream=True, enable_sync=False, use_alignment=False)
    ...
    def capture_loop(self):
        while self.running and not rospy.is_shutdown():
            color_image, _, _ = self.camera.get_frames()
    ...
    def shutdown(self):
        self.running = False
        self.thread.join()
        close_connected_cameras(self.camera)
